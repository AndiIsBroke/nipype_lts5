...
#import cmtk.parcellation as parcellation
#import numpy as np
...
#else:
from cmtklib.parcellation import create_annot_label, create_roi, create_wm_mask, crop_and_move_datasets
...
class ParcellateInputSpec(BaseInterfaceInputSpec):
    ...
    parcellation_scheme = traits.Enum('lausanne2008',['lausanne2008','freesurferaparc'], usedefault = True)
    scales = traits.List(traits.Str,desc='Scales to use.')
...
class Parcellate(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        iflogger.info("=============================================")
        create_annot_label(self.inputs.subject_id, self.inputs.subjects_dir, self.inputs.scales)
        create_roi(self.inputs.subject_id, self.inputs.subjects_dir, self.inputs.scales)
        create_wm_mask(self.inputs.subject_id, self.inputs.subjects_dir, self.inputs.scales)
    ...
    def _gen_outfilenames(self, basename):
        ...
        return filepaths
        #return prefix + '_' + self.inputs.scale + '.' + ext
